...
# Self-damping consolidation cadence (Hog1 loop). Ships ON at 0.25 (±25% swing
# with the last pass's activity); 0.0 disables. See engine/triggers.py.
CONSOLIDATION_INTERVAL_ADJUSTMENT = 0.25

# Sleep Cycle promotion scoring (was hardcoded in consolidation.py pre-1.22.6).
# Kept at the historical values so default promotion behavior is unchanged;
# re-tune only with R1 harness evidence. Mirror in templates/config.json.
PROMOTION_ACCESS_DIVISOR = 10.0        # access_count / divisor, capped at 1.0
PROMOTION_RECENCY_WINDOW = 30.0        # step_diff / window, recency floors at 0
PROMOTION_SEVERITY_CRITICAL = 1.0
PROMOTION_SEVERITY_MAJOR = 0.6
PROMOTION_SEVERITY_MINOR = 0.3
PROMOTION_WEIGHT_ACCESS = 0.4
PROMOTION_WEIGHT_SEVERITY = 0.4
PROMOTION_WEIGHT_RECENCY = 0.2
PROMOTION_SCORE_THRESHOLD = 0.5        # candidacy cutoff
PROMOTION_FORCE_CRITICAL = True        # always a candidate if severity == critical
PROMOTION_FORCE_ACCESS = 5             # always a candidate if access_count > this

# Staleness tiers by lines-changed since an entry's commit. The `minor` boundary
# re-uses the existing AUTO_LEARN_STALENESS_THRESHOLD (500) so nothing regresses;
# these add graded moderate/severe cutoffs on top. Mirror in templates/config.json.
STALENESS_MODERATE_THRESHOLD = 1500
STALENESS_SEVERE_THRESHOLD = 5000
...
ADAPTIVE_USEFULNESS_GAIN = 0.1   # max downward offset from access-based value
...
EVALUATE_MAX_PER_TURN = 5

CAPTURE_ENABLED = True
CAPTURE_ALWAYS_LOG = True
CAPTURE_MAX_SUMMARIES = 20
CAPTURE_MODE = "heuristic"
CAPTURE_NONE_LOG_REQUIRES_SIGNAL = True
VALID_CAPTURE_MODES = {"online", "offline", "heuristic"}
...
DEFAULTS = {
    ...
    "SLEEP_CYCLE_UNRESOLVED_THRESHOLD": SLEEP_CYCLE_UNRESOLVED_THRESHOLD,
    "CONSOLIDATION_INTERVAL_ADJUSTMENT": CONSOLIDATION_INTERVAL_ADJUSTMENT,
    "PROMOTION_ACCESS_DIVISOR": PROMOTION_ACCESS_DIVISOR,
    "PROMOTION_RECENCY_WINDOW": PROMOTION_RECENCY_WINDOW,
    "PROMOTION_SEVERITY_CRITICAL": PROMOTION_SEVERITY_CRITICAL,
    "PROMOTION_SEVERITY_MAJOR": PROMOTION_SEVERITY_MAJOR,
    "PROMOTION_SEVERITY_MINOR": PROMOTION_SEVERITY_MINOR,
    "PROMOTION_WEIGHT_ACCESS": PROMOTION_WEIGHT_ACCESS,
    "PROMOTION_WEIGHT_SEVERITY": PROMOTION_WEIGHT_SEVERITY,
    "PROMOTION_WEIGHT_RECENCY": PROMOTION_WEIGHT_RECENCY,
    "PROMOTION_SCORE_THRESHOLD": PROMOTION_SCORE_THRESHOLD,
    "PROMOTION_FORCE_CRITICAL": PROMOTION_FORCE_CRITICAL,
    "PROMOTION_FORCE_ACCESS": PROMOTION_FORCE_ACCESS,
    "STALENESS_MODERATE_THRESHOLD": STALENESS_MODERATE_THRESHOLD,
    "STALENESS_SEVERE_THRESHOLD": STALENESS_SEVERE_THRESHOLD,
    ...
    "ADAPTIVE_USEFULNESS_GAIN": ADAPTIVE_USEFULNESS_GAIN,
    ...
    "CAPTURE_ALWAYS_LOG": CAPTURE_ALWAYS_LOG,
    "CAPTURE_MAX_SUMMARIES": CAPTURE_MAX_SUMMARIES,
    "CAPTURE_MODE": CAPTURE_MODE,
    "CAPTURE_NONE_LOG_REQUIRES_SIGNAL": CAPTURE_NONE_LOG_REQUIRES_SIGNAL,
    "VALID_CAPTURE_MODES": VALID_CAPTURE_MODES,
