...
def validate_entry(entry, ctx):
    ...
    if (
        valid_source_agents is not None
        and entry["source_agent"] not in valid_source_agents
        and entry["source_agent"] != "system"
    ):
        errors.append(
            f"source_agent must be one of: {', '.join(sorted(valid_source_agents))}"
        )
