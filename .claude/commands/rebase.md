...
```powershell
"$([DateTime]::UtcNow.ToString('o')) /rebase" | Set-Content ".git/.claude-git-lock"
```
...
**Note:** `git rebase -i` opens an interactive editor that Claude Code can't control directly. Use `GIT_SEQUENCE_EDITOR` to automate it, or ask the user to run it in their terminal.
