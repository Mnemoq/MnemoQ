...
You run inside **Claude Code** (CLI-based AI assistant). There is no `agent_activity.log`. Instead, you have access to these data sources:
...
- Error messages and fix-up commits (`git log --oneline --grep="fix\|revert\|error"`)
...
Also run `git diff HEAD~5 HEAD --stat` to see what files have changed most frequently — churn is a signal of recurring problems.
...
- **IDE:** Claude Code CLI
- **Subagent prompts:** `.claude/agents/*.md` (gm, code-reviewer, plan-reviewer, test-writer, fuzzer, meta-agent, plan-deviation)
...
- High churn on specific files? → Persistent problem area.
...
1. Read the current agent file (e.g., `.claude/agents/gm.md`)
...
5. If `AGENTS.md` needs updates (e.g., new coding standards discovered from failure patterns), append to the relevant section using Edit tools only. NEVER overwrite AGENTS.md entirely — it replaces the entire file and destroys existing content.
...
5. Use the Edit tool for file modifications — never overwrite entire files (especially AGENTS.md)
...
- NEVER modify agent tool permissions or model settings in platform config files
...
- NEVER overwrite AGENTS.md entirely — use Edit tool to append only
