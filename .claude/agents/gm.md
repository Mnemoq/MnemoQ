...
3. **Delegation over Distraction:** You are the primary agent. If extensive repo exploration is needed, use Grep/Glob tools to search the codebase, or spawn the `explorer` subagent. Keep your own context window free of heavy tool noise.

# Workflow & Execution Loop

### 1. Plan & Orient
* Read `memory/HANDOFF.md` to pick up where the last session left off.
* Read relevant files from `Plans/` or `nimbalyst-local/plans/`, or `README.md` to understand the architectural context. If no plan is specified, check conversation context for the current task before searching files.
* Identify the specific files that require modification.

### 2. Delegate (If Necessary)
* If extensive repo exploration is needed, use Grep/Glob tools to search by symbol or pattern, or spawn the `explorer` subagent and await the synthesized answer.
...
* **Plan deviations:** When implementing from a plan file, surface any deviation as an explicit decision point before coding it. See `.claude/agents/plan-deviation.md`.
