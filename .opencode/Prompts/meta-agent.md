...
- **Subagent prompts:** `.windsurf/workflows/*.md` (gm, code-reviewer, plan-reviewer, test-writer, fuzzer, meta-agent, plan-deviation)
...
- **Memory system:** `src/mnemoq/cli.py` (retrieval + logging), `memory/learnings.jsonl`, `memory/metrics.jsonl`
...
3. Append new rules with timestamp and justification:
   ```
   - [2026-06-25] NEVER skip schema validation when adding new entry types — always update validate_entry() in src/mnemoq/engine/validation.py
   ```
...
5. If `AGENTS.md` needs updates (e.g., new coding standards discovered from failure patterns), append to the relevant section using `edit` or `multi_edit` tools only. NEVER use `write_to_file` on AGENTS.md — it replaces the entire file and destroys existing content.
...
5. Use the `edit` or `multi_edit` tools for file modifications — never overwrite entire files (especially AGENTS.md — see Safety Constraints)
...
```
python -m mnemoq.cli --log-file <temp-json-path>
```
...
- NEVER modify agent tool permissions or model settings in platform config files (`opencode.json`, `.cursor/rules/`, etc.)
- NEVER add more than 5 rules per evolution cycle
- NEVER edit `memory/SYSTEM_INVARIANTS.md` — invariants are immutable
- NEVER edit `memory/learnings.jsonl` directly — use `python -m mnemoq.cli --log-file`
- NEVER use `write_to_file` on AGENTS.md — it replaces the entire file. Use `edit` or `multi_edit` to append only
