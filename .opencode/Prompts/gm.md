...
* Read relevant `.opencode/plans/` or `README.md` files to understand the architectural context. If no plan is specified, check conversation context for the current task before searching files.
...
* **Before starting any task:**
  * Run `python -m mnemoq.cli --step <N> --components <CompA,CompB> --files <file1,file2> --domain <domain>` to retrieve relevant learnings.
...
* **After completing any task:**
  * If you discovered something non-obvious (bug, race condition, ordering constraint, optimization, architectural pattern), log it via:
    ```
    python -m mnemoq.cli --log-file <path-to-temp-json>
    ```
  ...
  * **Amending an entry:** `python -m mnemoq.cli --update <ts> --log-file <path>` — full entry required (all 11 fields), not a partial delta.
  * **Resolving an entry:** `python -m mnemoq.cli --resolve <ts>` — sets `resolved: true` only.
  * **Timestamp discovery:** `ts` is printed in `DUPLICATE` output. Otherwise: run `python -m mnemoq.cli --step <N> --components <...>` and check `DUPLICATE` output.
...
* **Session End (close the loop):**
  * Before handing off, emit a structured summary of the session and run the evaluator so learnable moments are auto-logged without hand-building each entry:
    * `python -m mnemoq.cli --evaluate-file <summary>.json`
  ...
  * Auto-learn runs automatically after each commit if the post-commit hook is installed (`python -m mnemoq.cli --install-hooks`, once per clone).
