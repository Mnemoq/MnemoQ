...
### Engine Core
`src/mnemoq/engine/` — 20 modules, each with `*_core()` (returns dict) and `handle_*()` (prints output) convention.

### Entry Points
`cli.py` (thin dispatcher), `mcp_main.py`, `scaffold.py`, `update.py`.

### SDK
`src/mnemoq/sdk/client.py` — `MemoryClient` with local/remote transports.

### Dashboard
`src/mnemoq/dashboard/static/` — web assets.

### Tests
`tests/` — `test_memory.py` (CLI integration), `test_server.py` (HTTP API), `test_pure_functions.py` (unit), `test_sdk.py`, `test_triggers.py`.

### Templates
`templates/` — config.json, prompts/, windsurf workflows, cursor-rules/.
