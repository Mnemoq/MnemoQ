...
# Universal schema constraints ΓÇö not configurable per-project.
# 
...
def load_config():
    ...
    Output contract:
    {
        ...
        "VALID_DOMAINS": set,             # null in config ΓåÆ None in dict
        "VALID_SOURCE_AGENTS": set,       # null in config ΓåÆ None in dict
        "VALID_RETRIEVAL_ONLY_AGENTS": set,  # null in config ΓåÆ None in dict
        
        # Domain mappings (for profile context)
        "DOMAIN_MAPPINGS": dict,          # null or {} in config ΓåÆ None in dict
        
        # Step bound
        "MAX_STEP": int,                  # null in config ΓåÆ None in dict
    }
...
def handle_review_agents(current_step, threshold):
    ...
    if not sections:
        ...
        if not os.path.exists(_get_paths().agents_md_path):
            ...
            print("WARNING: AGENTS.md has no ## headings ΓÇö nothing to report.")
        return 0
    ...
    if active:
        print("### ACTIVE ΓÇö Referenced by learnings")
    ...
    if cold:
        print("### COLD ΓÇö No references in last {} steps".format(threshold))
        for heading, count in cold:
            print(f"- {heading.lower().replace(' ', '-')} (0 refs) ΓÇö may be foundational or stale")
        print()

    # UNMATCHED learnings
    if unmatched_learnings:
        print("### UNMATCHED ΓÇö Learnings with no section match")
...
def handle_log(json_str):
    ...
    if similarity >= 0.7:
        ...
        print(f"DUPLICATE ΓÇö existing entry matches (similarity: {similarity:.2f}):")
    ...
    if 0.4 <= similarity < 0.7:
        if actions_oppose(entry["action"], best_match["action"]):
            ...
            print(f"CONFLICT ΓÇö potential contradiction detected (similarity: {similarity:.2f}):")
...
def handle_retrieval(current_step, task_components, task_files, task_domain):
    ...
    print("## ΓÜá WARNINGS ΓÇö Read before starting")
    if warnings:
    ...
    if escalations:
        print("\n## ΓÜí ESCALATION ΓÇö Critical entries unresolved for 30+ steps")
    ...
    if unresolved_count > 50 or current_step in sleep_cycle_steps:
        if unresolved_count > 50:
            print(f"\n## SLEEP CYCLE DUE ΓÇö {unresolved_count} unresolved entries exceed threshold of 50")
        if current_step in sleep_cycle_steps:
            print(f"\n## SLEEP CYCLE DUE ΓÇö Sprint boundary at step {current_step} ({unresolved_count} unresolved entries)")
...
def handle_stats():
    ...
    if unresolved > 50:
        print(f"\n## SLEEP CYCLE DUE ΓÇö {unresolved} unresolved entries exceed threshold of 50")
        print("Run the Sleep Cycle per AGENTS.md ## Memory before starting new work.")
...
def infer_sprint_number(entries):
    ...
    Examples: max_step=11 ΓåÆ sprint 2, max_step=20 ΓåÆ sprint 2, max_step=21 ΓåÆ sprint 3
    """
    if not entries:
        return 1
...
def handle_consolidate(sprint_number=None, confirm_reset=False, force=False):
    ...
    if not unresolved:
        print("ΓÜá No unresolved entries to consolidate. Archive not created.")
        return 0
    ...
    if os.path.exists(archive_path) and not force:
        print(f"ΓÜá Warning: {archive_path} already exists.")
    ...
    print(f"Γ£ô Archived {len(unresolved)} entries to {archive_path}")
    ...
    # Generate report
    print("\n" + "=" * 60)
    print(f"## SLEEP CYCLE REPORT ΓÇö Sprint {sprint_number}")
    print("=" * 60)

    # Promotion candidates
    print(f"\n## ≡ƒÄ» PROMOTION CANDIDATES ({len(candidates)} entries)")
    ...
    # Contradictions
    print(f"\n## ΓÜö∩╕Å CONTRADICTIONS ({len(contradictions)} entries)")
    print("The following entries propose superseding existing invariants via the Challenge Protocol.")
    print("Review carefully ΓÇö these may indicate outdated rules or necessary architectural changes.\n")
    ...
    print("- Chronic quarantine from one agent ΓåÆ meta_learning signal (agent doesn't understand schema)")
    print("- Repeated validation errors ΓåÆ need for better documentation or examples")
    ...
    if stale_count > 0 or error_count > 0:
        ...
        for is_stale, lines_changed, error, entry in stale_entries:
            if error:
                ...
                print(f"**Status:**  Could not check staleness ΓÇö {error}")
                ...
                print(f"**Status:**  HIGH CHURN ΓÇö verify trigger/action still hold")
                print(f"**Entry:** {entry.get('trigger', '')}")
                print()
                idx += 1
    else:
    ...
    if agents_suggestions:
        ...
        for entry in agents_suggestions:
            ...
            print(f"  ΓåÆ Suggests reviewing AGENTS.md for potential updates")
        print()
    ...
    print(f"Γ£ô Session saved. Run --confirm-reset within {SESSION_EXPIRY_MINUTES} minutes to clear learnings.jsonl.")
...
def handle_confirm_reset():
    ...
    print(f"Γ£ô Session valid (sprint {sprint}, started {ts.strftime('%H:%M:%S')})")

    # Clear learnings.jsonl
    with open(_get_paths().learnings_path, "w", encoding="utf-8") as f:
        pass  # Empty file

    print("Γ£ô learnings.jsonl reset. Sprint complete.")
