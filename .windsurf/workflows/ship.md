...
**Branch check (run before anything else):**
...
- **If on `main` or `master` AND the working tree is clean** (user already committed directly to main): **stop**. Tell the user:
  > *"You have commits directly on main. Create a feature branch and cherry-pick your commits onto it, then re-run /ship."*
  Do not proceed.

- **If on `main` or `master` AND there are uncommitted changes**: ask the user for a branch name and create it before committing:
...
**If the working tree is already clean** (user pre-committed, and branch check above passed), skip `/commit` and jump straight to Step 2. Tell the user: *"No uncommitted changes — skipping commit step."*
